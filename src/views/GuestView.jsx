...
import { Container, Typography, CircularProgress, Box, useMediaQuery, useTheme, Button, Paper } from '@mui/material';
...
const GuestView = () => {
  ...
  const processGuests = (guestsData) => {
    ...
    setAllergies(Array.from(uniqueAllergies).map(allergy => JSON.parse(allergy)));
    ...
    <Container maxWidth={isSmallScreen ? 'sm' : 'xl'}>
      <Paper elevation={1} sx={{ padding: 2, marginBottom: 2 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant={isSmallScreen ? "h5" : "h4"} component="h1" style={{color:"black"}}>
            Invitados
          </Typography>
          <Button variant="contained" color="primary" onClick={handleCreateGuest}>
